...
export async function saveImageMetas(images: ImageMeta[]): Promise<void> {
  await migrateLegacyImages();
  const projectMap = new Map<string, ImageMeta[]>();
...
export async function getImageDataUrl(imageId: string): Promise<string | undefined> {
  await migrateLegacyImages();
  const localDataUrl = await get<string>(imageDataKey(imageId));
  if (localDataUrl) return localDataUrl;

  const serverDataUrl = await getServerImageDataUrl(imageId);
  if (serverDataUrl) {
    await set(imageDataKey(imageId), serverDataUrl);
  }
...
export async function getImagesWithData(images: ImageMeta[]): Promise<ImageItem[]> {
  const localDataUrls = await getManyInBatches<string>(images.map((image) => imageDataKey(image.id)));
  const imageItems: ImageItem[] = [];

  for (const [index, image] of images.entries()) {
    const dataUrl = localDataUrls[index] || await getImageDataUrl(image.id);
    if (dataUrl) {
      imageItems.push({ ...image, dataUrl });
    }
  }
